(window.constructQZFL = function() {
	...
	(function() {
		var ua = QZFL.userAgent = {}, agent = navigator.userAgent,
		...
		else ua.ie = 6; if (!(ua.macs = agent.indexOf("Mac OS X") > -1)) {
		...
		extend: function() {
			...
			for (; i < len; i++) if ((opts = arguments[i]) != null) for (var name in opts) {
						...
						if (deep && copy && typeof copy === "object" && !copy.nodeType) {
							if (src) clone = src;
							else if (QZFL.lang.isArray(copy)) clone = [];
							else if (QZFL.object.getType(copy) === "object") clone = {};
							else clone = copy;
	...
	typeof QZFL.config.domain == "undefined" && (QZFL.config.domain =
		"qzs.qq.com");
	...
	QZFL.css = {
		...
		rgb2hsl: function(r, g, b) {
			...
			if (max != min) {
				if (max == red) result.h = (t = 60 * ((green -
						blue) / (max - min))) < 0 ? t + 360 : t;
				else if (max == green) result.h = 60 * ((blue - red) / (max - min)) + 120;
				else if (max == blue) result.h = 60 * ((red - green) / (max - min)) + 240;
				if (result.l <= 0.5) result.s = (max - min) / (2 * result.l);
				else if (0.5 < result.l) result.s = (max - min) / (2 - 2 * result.l);
		...
		getRulesBySheet: function(sheetId) {
			var ss =
				typeof sheetId == "object" ? sheetId : QZFL.css.getStyleSheetById(sheetId),
				rs = {}, head, base;
		...
		getRuleBySelector: function(sheetId, selector) {
			...
			for (var i = 0, len = _rs.length; i < len; ++i) if (selector ==
					_rs[i].selectorText.toLowerCase()) return _rs[i];
			return null
		},
		...
		hasClassName: function(elem, name) {
			return elem && name ? elem.classList ? elem.classList.contains(name) : name && (" " + elem.className + " ").indexOf(" " +
				name + " ") > -1 : false
		},
		...
		swapClassName: function(elems, a, b, _isRep) {
			if (elems &&
				...
				for (var elem, i = 0, l = elems.length; i < l; ++i) if ((elem = elems[i]) && elem.nodeType == 1) if (QZFL.css.hasClassName(elem, a)) QZFL.css.updateClassName(elem, a, b);
						else
				if (!_isRep && QZFL.css.hasClassName(elem, b)) QZFL.css.updateClassName(elem, b, a)
			}
		},
		toggleClassName: function(elem, name) {
			...
			if (_s.hasClassName(elem, name)) _s.updateClassName(elem,
					name);
			else _s.updateClassName(elem, null, name)
		}
	};
	QZFL.dom = {
		getById: function(id) {
		...
		removeElement: function(elem) {
			if (elem = QZFL.dom.get(elem)) {
				...
				elem.removeNode ? elem.removeNode(true) :
		...
		getElementsByClassName: function(className, tagName, context) {
			return QZFL.selector((tagName ||
				"") + "." + className, QZFL.dom.get(context))
		},
		...
		createElementIn: function(tagName, elem, insertFirst, attrs) {
			var _e = (elem = QZFL.dom.get(elem) || document.body).ownerDocument.createElement(tagName || "div"),
				k;
			if (typeof attrs == "object") for (k in attrs) if (k ==
						"class") _e.className = attrs[k];
					else
			if (k == "style") _e.style.cssText = attrs[k];
			else _e[k] = attrs[k];
		...
		getStyle: function(el, property) {
			...
			else return el.currentStyle[property] ||
					el.style[property]
		},
		setStyle: function(el, properties, value) {
			var DOM = QZFL.dom;
			if (!(el = DOM.get(el)) || el.nodeType != 1) return false;
			var tmp, bRtn = true,
				re;
		...
		convertStyle: function(el, prop, value) {
			var DOM = QZFL.dom,
				tmp, rexclude = /z-?index|font-?weight|opacity|zoom|line-?height/i,
				w3cMode;
		...
		getPosition: function(elem) {
			...
			if (box = QZFL.dom.getRect(elem)) {
				...
				if (s = QZFL.dom.getScrollTop(doc)) box.top += s, box.bottom += s;
				return box
			}
		},
		...
		getScrollLeft: function(doc) {
			var _doc = doc || document;
			return _doc.defaultView && _doc.defaultView.pageXOffset || Math.max(_doc.documentElement.scrollLeft,
		...
		setScrollLeft: function(value, doc) {
			var _doc = doc || document;
			_doc[_doc.compatMode ==
				"CSS1Compat" && !QZFL.userAgent.webkit ? "documentElement" : "body"].scrollLeft = value
		},
		...
		getClientWidth: function(doc) {
			var _doc = doc || document;
			return _doc.compatMode == "CSS1Compat" ? _doc.documentElement.clientWidth :
		...
		getElementsByTagNameNS: function(node, ns, tgn) {
			node = node || document;
			var res = [];
			if (node.getElementsByTagNameNS) return node.getElementsByTagName(ns +
					":" + tgn);
		...
		insertAdjacent: function(elem, where, html, isText) {
			...
			if (QZFL.lang.isElement(elem) && pos[where] && (QZFL.lang.isString(html) || QZFL.lang.isElement(html))) {
				if (elem.insertAdjacentHTML && elem.insertAdjacentElement && elem.insertAdjacentText) elem["insertAdjacent" + (typeof html == "object" ? "Element" : isText ? "Text" : "HTML")](pos[where], html);
				else {
	...
	QZFL.event = {
		...
		addEvent: function(obj, eventType, fn, argArray) {
			...
			if (!(l = QZFL.event._eventListDictionary[obj.eventsListUID])) l =
			...
			else res = false; if (res) handlers[fn.__elUID] = cfn;
			return res
		},
		trigger: function(obj, eventType) {
			var l = obj && QZFL.event._eventListDictionary[obj.eventsListUID],
				...
				i;
			if (handlers) try {
					for (i in handlers) handlers[i].call(window, {})
			} catch (evt) {
		...
		removeEvent: function(obj, eventType, fn) {
			...
			if (obj.removeEventListener) {
				...
				res = true
			} else return false; if (res && r && r[fn.__elUID]) delete r[fn.__elUID];
			return res
		},
		purgeEvent: function(obj, type) {
			var l, h;
			if (obj.eventsListUID && (l = QZFL.event._eventListDictionary[obj.eventsListUID]) && l[type] && (h = l[type].handlers)) for (var k in h) if (obj.removeEventListener) obj.removeEventListener(type, h[k], false);
					else
			if (obj.detachEvent) obj.detachEvent("on" + type, h[k]);
		...
		getEvent: function(evt) {
			...
			if (!evt) {
				...
				while (c && ct < _s.MAX_LEVEL) {
					if (c.arguments &&
						(evt = c.arguments[0]) && typeof evt.button != "undefined" && typeof evt.ctrlKey != "undefined") break;
					++ct;
					c = c.caller
				}
			}
		...
		getButton: function(evt) {
			...
			if (!e) return -1;
			if (QZFL.userAgent.ie) return e.button - Math.ceil(e.button / 2);
		...
		getRelatedTarget: function(ev) {
			...
			if (ev.type == "mouseover") t = ev.fromElement;
			else;
			return t
		},
		...
		_bindReady: function() {
			...
			if (document.readyState ===
				"complete") return setTimeout(QZFL.event._readyFn, 1);
	...
	QZFL.queue = function() {
		...
		var _extend = {
			...
			shift: function(key) {
				...
				if (_q) return QZFL.queue._exec(_q.shift())
			},
			getLen: function(key) {
				return _queue[this._qz_queuekey ||
					key].length
			},
			run: function(key) {
				var _q = _queue[this._qz_queuekey || key];
				if (_q) _o.each(_q, QZFL.queue._exec)
			},
			timedChunk: function(key, conf) {
				...
				if (_q) {
					...
					setTimeout(function() {
						var _start = +new Date;
						do QZFL.queue.shift(key); while (QZFL.queue.getLen(key) > 0 && +new Date - _start < _conf.runTime);
			...
			_exec: function(value, key, source) {
				if (!value || _o.getType(value) != "function") {
					if (_o.getType(key) == "number") source[key] = null;
					return false
				}
				try {
					return value()
	...
	(function() {
		...
		QZFL.object.extend(QZFL.util, {
			copyToClip: function(text) {
				if (QZFL.userAgent.ie) return clipboardData.setData("Text",
						text);
				else {
			...
			commonDictionarySplit: function(s, esp, vq, eq) {
				var res = {}, l, ks, vs, t;
				if (!s ||
					typeof s != "string") return res;
				if (typeof esp != "string") esp = "&";
				if (typeof vq != "string") vq = "";
				if (typeof eq != "string") eq = "=";
				l = s.split(esp);
				if (l && l.length) for (var i = 0, len = l.length; i < len; ++i) {
						ks = l[i].split(eq);
						if (ks.length > 1) {
			...
			commonDictionaryJoin: function(o, esp, vq, eq, valueHandler) {
				...
				if (typeof vq != "string") vq = "";
				if (typeof eq != "string") eq = "=";
				for (var k in o) res.push(k + eq + vq + (typeof valueHandler == "function" ? valueHandler(o[k]) : o[k]) + vq);
				return res.join(esp)
			}
		})
	})();
	...
	QZFL.lang.getObjByNameSpace = function(ns, setup) {
		...
		try {
			for (var i = 0, len =
					l.length; i < len; ++i) {
				if (typeof r[l[i]] == "undefined") if (setup) r[l[i]] = {};
					else return;
				r = r[l[i]]
			}
			return r
	...
	QZFL.lang.objectClone = function(obj, preventName) {
		if (typeof obj == "object") {
			...
			for (var i in obj) if (i != preventName) res[i] = QZFL.lang.objectClone(obj[i], preventName);
			return res
	...
	QZFL.lang.obj2str = function(obj) {
		...
		if (typeof obj == "object") {
			...
			t = [];
			for (var i in obj) t.push((sw ? "" : '"' + QZFL.string.escString(i) + '":') + obj2str(obj[i]));
			t = t.join();
			return sw ? "[" + t + "]" : "{" + t + "}"
	...
	QZFL.lang.propertieCopy = function(s, b, propertiSet, notOverWrite) {
		...
		s = s || {};
		for (var p in l) if (!notOverWrite || !(p in
				s)) s[p] = l[p];
		return s
	};
	QZFL.lang.tryThese = function() {
		for (var i = 0, len = arguments.length; i < len; ++i) try {
				return arguments[i]()
		} catch (ign) {}
		return
	};
	QZFL.lang.chain = function(u, v) {
		...
		return function() {
			for (var i = 0, len = calls.length; i < len; ++i) if (calls[i] && calls[i].apply(null, arguments) === false) return false;
			return true
		}
	};
	...
	QZFL.pageEvents = function() {
		function _ihp() {
			...
			if (s && s.DEBUG) {
				...
				if (!isNaN(n)) QZFL.config.debugLevel = n
			}
		}
		function _bootStrap() {
			if (QZFL.event.onDomReady.isReady) setTimeout(_onloadHook, 1);
			else if (document.addEventListener) document.addEventListener("DOMContentLoaded", _onloadHook, true);
			else {
				var src =
		...
		function _runHooks(hooks) {
			...
			if (isbeforeunload) if (warn) return warn;
				else QZFL.enviroment.loaded = false
		}
		...
		function pinit() {
			...
			if (t && t.DEBUG) QZFL.config.debugLevel = 2
		}
		return {
			onloadRegister: _lr,
	...
	QZFL.string.parseXML = function(text) {
		...
		if (window.ActiveXObject) {
			...
			doc.async = "false";
			doc.loadXML(text);
			if (doc.parseError.reason) return null
		} else {
	...
	QZFL.string.timeFormatString = function(date, ptn, baseTime) {
		...
		if (!ptn) {
			...
			for (var i = 0, len = unt.length; i < len; ++i) {
				t = map[unt[i]];
				if (delta > t[1]) return Math.floor(delta / t[1]) + t[2]
			}
			return "\u521a\u521a"
		} else return ptn.replace(me._re, function(a, b, c) {
				...
				} else return Math.floor(Math.abs(date - baseTime) / map[b][1])
			})
	};
	...
	(function() {
		...
		var Sizzle = function(selector, context, results, seed) {
			...
			if (!contextXML) {
				...
				if (speedUpMatch) {
					if (context.nodeType === 1 ||
						context.nodeType === 9) if (speedUpMatch[1]) return makeArray(context.getElementsByTagName(selector), results);
						else
					if (speedUpMatch[2] || speedUpMatch[4] && speedUpMatch[5]) if (context.getElementsByClassName && speedUpMatch[2]) return makeArray(context.getElementsByClassName(speedUpMatch[2]), results);
						else {
					...
					if (context.nodeType === 9) if ((selector === "body" || selector.toLowerCase() === "body") && context.body) return makeArray([context.body], results);
						else
					if (speedUpMatch[3]) return (tmpVar = context.getElementById(speedUpMatch[3])) ? makeArray([tmpVar], results) : makeArray([], results)
				}
			}
			do {
				chunker.exec("");
			...
			if (parts.length > 1 && origPOS.exec(selector)) if (parts.length ===
					2 && Expr.relative[parts[0]]) set = posProcess(parts[0] + parts[1], context);
				else {
					...
					if (context) {
						...
						while (parts.length) {
							...
							else pop = parts.pop(); if (pop == null) pop = context;
			...
			if (context && context.nodeType === 1) for (i = 0; checkSet[i] != null; i++) {
			...
			} else makeArray(checkSet, results); if (extra) {
				Sizzle(extra,
		...
		if (document.querySelectorAll)(function() {
				...
				Sizzle = function(query, context, extra, seed) {
					...
					if (!seed && !Sizzle.isXML(context)) {
						...
						if (match &&
							(context.nodeType === 1 || context.nodeType === 9)) if (match[1]) return makeArray(context.getElementsByTagName(query), extra);
							else
						...
						if (context.nodeType === 9) {
							...
							else nid = nid.replace(/'/g, "\\$&"); if (relativeHierarchySelector && hasParent) context = context.parentNode;
							try {
		...
		(function() {
			...
			if (matches) Sizzle.matchesSelector = function(node, expr) {
		...
		if (document.documentElement.compareDocumentPosition) Sizzle.contains = function(a, b) {
				...
				else if (!b || b.nodeType == 9) return false;
				else if (b === a) return true;
				else return Sizzle.contains(a, b.parentNode)
		};
	...
	(function() {
		...
		_handler.prototype = {
			_init: function(selector,
				context) {
				if (QZFL.lang.isString(selector)) this.elements = QZFL.selector(selector, context);
				else if (selector instanceof QZFL.ElementHandler) this.elements = selector.elements.slice();
				else if (QZFL.lang.isArray(selector)) this.elements = selector;
				else if (selector && (selector.nodeType && selector.nodeType !== 3 && selector.nodeType !== 8 || selector.setTimeout)) this.elements = [selector];
				else this.elements = []
			},
	...
	QZFL.object.each(["onClick", "onMouseDown", "onMouseUp", "onMouseOver",
			"onMouseMove", "onMouseOut", "onFocus", "onBlur", "onKeyDown", "onKeyPress", "onKeyUp"
	], function(name, index) {
	...
	QZFL.element.extendFn({
			...
			if (QZFL.object.getType(value) == "array") {
				...
				this.each(function(el) {
					if (/radio|checkbox/.test(el.type)) el.checked =
							el.nodeType && ("\x00" + _v.indexOf(el.value.toString() + "\x00") > -1 || _v.indexOf("\x00" + el.name.toString() + "\x00") > -1);
					else if (el.tagName == "SELECT") QZFL.object.each(el.options, function(e) {
		...
		getVal: function(index) {
			...
			if (_e) if (_e.tagName == "SELECT") {
					...
					if (_e.type == "select-one") _v.push(_e.value);
					else QZFL.object.each(_e.options, function(e) {
	...
	QZFL.effect = {
		...
		toggle: function(elem, opts, cb) {
			var o = QZFL.effect;
			opts = opts || {};
			if (o._isHidden(elem)) o.stop(elem).show(elem, opts, cb);
			else o.stop(elem).hide(elem, opts, cb)
		},
		slideDown: function(elem, opts) {
			var d = QZFL.dom,
				attrs, o = QZFL.effect,
				toValue = {}, _obj = QZFL.object,
			...
			if (attrs) {
				...
				o.run(elem, attrs, _obj.extend(opts, {
					duration: duration || 1E3,
					start: function() {
						if (attrs && !opts.noClear) _obj.each(attrs, function(v, i) {
		...
		slideToggle: function(elem, opts) {
			var o = QZFL.effect,
				status, opts = opts || {};
			if (o._isHidden(elem)) o.stop(elem).slideDown(elem, opts);
			else if (elem._slideid) {
				status =
					o._slideArray[elem._slideid].status;
				if (status == "up") {
					opts.noClear = 1;
			...
			} else o.stop(elem).slideUp(elem, opts)
		},
		_checkVerticalStyle: function(elem, opts) {
			...
			if (!elem._slideid) elem._slideid = ++o._uniqueID;
	...
	QZFL.cssTransfrom = function(elem, prop, opts) {
		...
		if (!o._running && prop) {
			o._conf = prop;
			o._duration = "duration" in
				opts ? opts.duration / 1E3 : 0.5;
	...
	QZFL.cssTransfrom.prototype = {
		...
		_addProperty: function(prop, conf) {
			...
			o._count++;
			dur = (typeof conf.duration != "undefined" ? conf.duration : o._duration) || 1.0E-4;
			attrs[prop] = {
				value: val,
				duration: dur,
			...
			compareVal =
				typeof val === "string" ? computed : parseFloat(computed);
		...
		stop: function() {
			...
			pps = cText.property;
			for (var i = 0; i < pps.length; i++) styleText.push(pps[i] + ":" + QZFL.dom.getStyle(elem, pps[i]));
	...
	QZFL.Tween.prototype.start = function() {
		...
		if (set) {
			...
			o.css = false;
			var t =
				new QZFL.tweenMaker(s, e, o.opts.duration, o.opts.interval || 15);
			t.onStart = function() {
				o.t = t;
				o.onMotionStart.apply(o)
			};
	...
	(function() {
		var _easeAnimate = function(_t, a1, a2, ease) {
			...
			n.onMotionChange = QZFL.event.bind(this, function() {
				var _p = arguments[2];
				QZFL.dom["set" + _t](this, typeof a1 != "number" ? _s[0] : _s[0] - _p / 100 * _v1, typeof a2 != "number" ? _s[1] :
					_s[1] - _p / 100 * _v2)
			});
			if (_t == "Size" && _reset) n.onMotionStop = QZFL.event.bind(this, function() {
		...
		var _easeScroll = function(top, left, ease) {
			if (this.nodeType == 9) var _stl = [QZFL.dom.getScrollTop(this),
					QZFL.dom.getScrollLeft(this)
				];
			else var _stl = [this.scrollTop, this.scrollLeft];
			var _st = _stl[0] - top;
			var _sl = _stl[1] - left;
			var n = new QZFL.Tween(this, "_p", QZFL.transitions[ease] || QZFL.transitions.regularEaseOut, 0, 100, 0.5);
			n.onMotionChange = QZFL.event.bind(this, function() {
		...
		QZFL.element.extendFn({
			...
			effectShow: function(effect, ease) {
				...
				if (effect == "resize") this.each(function(el) {
			...
			Promise = function() {
				...
				add: function(type, func) {
					...
					if (typeof func == "function") {
						if (this.status ==
							type) func.apply(window, this.eventList[type + "Datas"]);
						else this.eventList[type + "Funcs"].push(func);
				...
				trigger: function(type, datas) {
					...
					if (type in this._status) {
						...
						if (this.eventList.added) {
							...
							while (func = funcs.shift()) func.apply(window, datas)
						} else this.eventList[type + "Datas"] = datas;
						this.status = type
					}
				}
			}, _promise;
	...
	QZFL.object.extend(QZFL, {
		when: function(promise) {
			...
			updateFunc = function(index, data) {
				...
				if (!--remain) promise.resolve.apply(promise, datas)
			};
			if (length > 1) for (var i = 0; i < length; i++) {
					if (!promises[i].state) throw new Error("not a promise instance");
	...
	QZFL.XHR.path = "http://" + QZFL.config.resourceDomain + "/ac/qzfl/release/expand/xhr_base.js?max_age=864001", QZFL.XHR.prototype.send = function() {
		var _s = QZFL.XHR,
			fn;
		if (this._method == "POST" && !this._data) return false;
	...
	(function() {
		...
		function transdata(data) {
			var ar = [];
			if (typeof data == "object") for (var i in data) ar.push(i + "=" + encodeURIComponent(data[i]));
			else ar.push(data);
			return ar.join("&")
		}
		...
		function createXHR() {
			...
			else if (window.ActiveXObject) xhr = new ActiveXObject("Msxml2.XMLHTTP") || new ActiveXObject("Microsoft.XMLHTTP");
			return xhr
		}
		function setHeader(xhr, kv) {
			for (var i in kv) kv[i] && xhr.setRequestHeader(i, kv[i])
		}
		function processText(str) {
			if (str.indexOf("<script") > -1) {
				var lidx = str.lastIndexOf(")"),
					rst = str.substring(str.indexOf(".callback(") + 10, lidx == -1 ? str.length : lidx);
				return rst
			}
			return '""'
		}
			...
			if (isSameDomain(url)) {
				...
				xhr.onreadystatechange = function() {
					if (this.readyState == 4) if (this.status == 200) {
							...
							opt.onload.call(this, eval("(" + (opt.processText || processText)(this.responseText) + ")"));
				...
				xhr.onload = function() {
					...
					QZFL["FormSender"]._pluginsRunner("onRequestComplete", opt.instance ||
						instance)
				}
			}
	...
	QZFL.debug = {
		...
		showLog: function() {
			...
			if ( !! o) o.innerHTML = QZFL.string.nl2br(QZFL.string.escHTML(this.errorLogs.join("\n")))
		},
	...
	QZFL.JsLoader.prototype._load = function() {
		...
		return function(src, doc, opts) {
			var o = this,
				tmp, k, head = doc.head || doc.getElementsByTagName("head")[0] || doc.body,
				_new = false,
				_js;
			...
			_ae(_js, _ie && _ie < 10 ? "readystatechange" : "load", function() {
				if (!_js ||
					_ie && _ie < 10 && (typeof docMode == "undefined" || docMode < 10 ? _js.readyState != "loaded" : _js.readyState != "complete")) return;
				_ie && idp.push(_js.id);
				o.onload();
				!_ie && _rm(_js);
				_js = o = null
	...
	QZFL.imports = function(sources, succCallback, opts) {
		var errCallback,
			url, len, countId, counter, scb, ecb, i, isFn = QZFL.lang.isFunction;
	...
	QZFL.FormSender._pluginsRunner = function(pType, data) {
		var _s = QZFL.FormSender,
			...
			t = data,
			len;
		if (l && (len = l.length)) for (var i = 0; i < len; ++i) if (typeof l[i] == "function") t = l[i](t) || data;
		return t
	};
	QZFL.FormSender._clear = function(o) {
		o._sender = o._sender.callback = o._sender.errorCallback =
			null;
		if (QZFL.userAgent.safari || QZFL.userAgent.opera) setTimeout('QZFL.dom.removeElement(document.getElementById("_fp_frm_' + o.name + '"))', 50);
		else QZFL.dom.removeElement(document.getElementById("_fp_frm_" + o.name));
		o.endTime = +new Date;
	...
	QZFL.FormSender.prototype.send = function() {
		...
		(function(callback) {
			...
			while (true) {
				flag = false;
				try {
					_cur.document && _cur.document.domain == document.domain && (flag =
						true)
				} catch (_) {}
				if (flag) _pool.push(_cur);
				if (_cur == top) break;
				_cur = _cur.parent
			}
			while (_pool.length) if (callback(_cur = _pool.pop()) === true) return _cur;
			return null
		})(function(w) {
		...
		return function() {
			...
			if (this.charset == "utf-8") if (1 && win.QZFL && win.QZFL.FormSender && (isspeedup && win.QZFL.config.xhrProxyEnable && win.QZFL.config.xhrProxyEnable(realurl) == 1 || canUseXHR && canUseXHR(ajaxurl))) {
					...
					return this.postTime = +new Date,
					this.type = "xhr", win.QZFL.ajaxPost(ajaxurl, data, {
			...
			if (this._sender === null || this._sender === void 0) {
				...
				var errcallback = function(th) {
					...
					return function() {
						if (typeof th.resultArgs == "object") return;
						if (this.readyState == "complete" || typeof this.readyState == "undefined") if ("sended".indexOf(this.state) > -1) {
				...
				sender.onload = sender.onreadystatechange =
					errcallback;
			...
			if (!this.instanceForm) {
				...
				if (ie) ifrurl = 'javascript:document.open();document.domain="' + document.domain + '";var me=parent.QZFL.FormSender.instance["' + t.name + '"];document.write(me.ifrHTML);document.close();';
				ifrHTML = ifrHTML + '<script type="text/javascript">' +
					(ie && 'document.charset="' + t.charset + '"' || "") + ';document.domain="' + document.domain + '";frameElement.submited=void(0);frameElement.state="sending";<\/script></head><body>';
				...
				ifrHTML = ifrHTML + '</form><script type="text/javascript">var me=parent.QZFL.FormSender.instance["' +
	...
	QZFL.JSONGetter = function(actionURL, cname, data, charset, junctionMode) {
		if (QZFL.object.getType(cname) != "string") cname = "_jsonInstence_" + (QZFL.JSONGetter.counter + 1);
	...
	QZFL.JSONGetter._runFnQueue = function(q,
		resultArgs, th) {
		var f;
	...
	QZFL.JSONGetter.prototype.send = function(callbackFnName) {
		...
		var clear, cfn = QZFL.object.getType(callbackFnName) != "string" ? "callback" : callbackFnName,
			da = this._uri;
		if (this._data) da += (da.indexOf("?") < 0 ? "?" : "&") + (typeof this._data == "object" ? QZFL.JSONGetter.genHttpParamString(this._data) : this._data);
		da = QZFL.JSONGetter._pluginsRunner("srcStringHandler",
			da);
		if (this._jMode) {
		...
		if (QZFL.userAgent.ie && (typeof document.documentMode == "undefined" || document.documentMode < 10) && !(QZFL.userAgent.beta && navigator.appVersion.indexOf("Trident/4.0") > -1)) {
			...
			if (QZFL.userAgent.ie < 9) sender.onreadystatechange = function(th) {
					return function() {
						if (th._sender &&
							th._sender.readyState == "loaded") try {
								th._onError()
						} catch (ignore) {}
					}
			}(this);
			...
			var callback = function(th) {
				return function() {
					...
					QZFL.JSONGetter._runFnQueue(th._squeue, QZFL.JSONGetter.args,
						th);
	...
	QZFL.pingSender = function(url, t, opts) {
		...
		}, typeof opts.timeoutValue == "number" ? Math.max(100, opts.timeoutValue) : 5E3);
	...
	QZFL.media = {
		...
		getFlashHtml: function(flashArguments, requiredVersion, flashPlayerCID) {
			...
			for (var k in flashArguments) switch (k) {
					...
					case "style":
						...
						break;
					case "src":
						if (QZFL.userAgent.ie) _params.push('<param name="movie" value="', flashArguments.noSrc ? "" : flashArguments[k],
								'"/>');
			...
			if (QZFL.userAgent.ie) _attrs.push(' classid="clsid:', flashPlayerCID || "D27CDB6E-AE6D-11cf-96B8-444553540000", '"');
			else _attrs.push(' type="application/x-shockwave-flash"'); if (requiredVersion && requiredVersion instanceof QZFL.media.SWFVersion) {
				var _ver = QZFL.media.getFlashVersion().major,
					_needVer = requiredVersion.major;
				_attrs.push(' codeBase="http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab#version=',
					requiredVersion, '"')
			}
		...
		insertFlash: function(containerElement, flashArguments) {
			...
			else f.setAttribute("data", flashArguments.src);
			return true
		},
		getWMMHtml: function(wmpArguments, cid) {
			...
			if (wmpArguments["src"]) params.push('<param name="URL" value="',
					wmpArguments["src"], '" />');
			if (QZFL.userAgent.ie) return '<object classid="' + (cid || "clsid:6BF52A52-394A-11D3-B153-00C04F79FAA6") + '" ' + objArgm.join("") + ">" + params.join("") + "</object>";
			else return "<embed " + objArgm.join("") + "></embed>"
		}
	};
	QZFL.media.SWFVersion = function() {
		...
		if (typeof arguments[0] == "number") a = [arguments[0]];
		else a = [];
		else a = [];
	...
	QZFL.media.reduceImage = function() {
		...
		return function(type,
			...
			if (opts.img) if (!(opts.direction && opts.rate && opts.longSize && opts.shortSize)) r = QZFL.media.getImageInfo(function(o, p) {
				...
				else doReduce(opts.img, type, ew, eh, opts, opts.callback)
		}
	}();
	...
	QZFL.shareObject.create = function(path) {
		if (typeof path == "undefined") path = QZFL.config.defaultShareObject;
	...
	QZFL.shareObject.getValidSO = function() {
		...
		for (var i = 1; i < cnt + 1; ++i) if (QZFL.shareObject.instance["so_" + i] && QZFL.shareObject.instance["so_" + i]._ready) return QZFL.shareObject.instance["so_" + i];
		return null
	};
	...
	QZFL.shareObject.DataBase = function(soUrl) {
		if (QZFL.shareObject.refCount > 0) return QZFL.shareObject.instance["so_1"];
		this._ready = false;
	...
	QZFL.dragdrop = {
		...
		registerDragdropHandler: function(handler, target, opts) {
			...
			if (!_hDom.id) _hDom.id = "dragdrop_" + ++_s.count;
	...
	QZFL.widget.msgbox = {
		...
		_loadCss: function(s) {
			...
			s = s || th.cssPath;
			if (th.currentCssPath != s) QZFL.css.insertCSSLink(th.currentCssPath = s)
		},
	...
	QZFL.dialog._shellCall = function(pFnName, objInstance,
		args) {
		var _s = QZFL.dialog;
		QZFL.imports(_s.path, function(th) {
			return function() {
	...
	QZFL.dialog.shell = function(opts) {
		...
		if (!_s.base) QZFL.imports(_s.path)
	};
	QZFL.dialog.shell.prototype.getZIndex = function() {
		return this.zIndex ||
			6E3 + QZFL.dialog.count
	};
	(function(fl) {
		for (var i = 0, len = fl.length; i < len; ++i) QZFL.dialog.shell.prototype[fl[i]] = function(pName) {
				return function() {
	...
	QZFL.widget.Confirm = function(title, content, opts) {
		if (typeof opts != "undefined" && typeof opts != "object") opts = {
				type: opts,
				tips: arguments[3]
		};
		opts = opts || {};
		...
		if (!_s.iconMap) QZFL.imports(_s.path)
	};
	...
	(function(fl) {
		for (var i = 0, len = fl.length; i < len; ++i) QZFL.widget.Confirm.prototype[fl[i]] = function(pName) {
				return function() {
					QZFL.widget.Confirm._shellCall(pName,
						this, arguments)
				}
		}(fl[i])
	})(["hide", "show"]);
	...
	QZFL.maskLayout = function() {
		var masker = null,
			...
			qml = function(zi, doc, opts) {
				++count;
				if (masker) return count;
				zi = zi || 5E3;
				doc = doc || document;
				opts = opts || {};
				var t = parseFloat(opts.opacity, 10);
				opts.opacity = isNaN(t) ?
					0.2 : t;
				...
				masker.style.cssText = "background-color:#000;filter:alpha(opacity=" + 100 * opts.opacity + ");opacity:" + opts.opacity + ";position:fixed;_position:absolute;left:" + opts.left + "px;top:" + opts.top + "px;z-index:" + zi + ";width:100%;height:" + QZFL.dom[QZFL.userAgent.ie <
					7 ? "getScrollHeight" : "getClientHeight"](doc) + "px;";
				return count
			};
		qml.setOpacity = function(ov) {
			if (masker && ov) QZFL.dom.setStyle(masker, "opacity", ov)
		};
		qml.getRef = function() {
			return masker
		};
	...
	QZFL.fixLayout = {
		...
		_init: function() {
			...
			this._isInit = true;
			if (this._isIE6) QZFL.event.addEvent(document.compatMode == "CSS1Compat" ? window : document.body, "scroll", QZFL.event.bind(this, this._onscroll))
		},
		create: function(html, isBottom, layerId, noFixed, options) {
			...
			if (layerId) tmp.id = layerId;
			this._layoutCount++;
			_c =
				this._layoutDiv[this._layoutCount] = QZFL.dom.createElementIn("div", this._fixLayout, false, tmp);
		...
		moveTop: function(layoutId) {
			if (!this._layoutDiv[layoutId].isTop) with(this._layoutDiv[layoutId]) {
		...
		_doScroll: function() {
			...
			for (var k in o._layoutDiv) {
				...
				if (_item.isTop) o._layoutDiv[k].style.marginTop = QZFL.dom.getScrollTop() + "px";
				else {
	...
	QZFL.widget.bubble = {
		...
		count: 0,
		hide: function(id) {
			if (QZFL.widget.tips) QZFL.widget.tips.close(id)
		},
		hideAll: function() {
			if (QZFL.widget.tips) QZFL.widget.tips.closeAll()
		}
	};
	...
	QZFL.widget.tips = {
		...
		resize: function(id) {
			QZFL.imports(QZFL.widget.tips.path, function() {
				if (QZFL.widget.tips) QZFL.widget.tips.resize && QZFL.widget.tips.resize(id)
			})
		}
	};
	...
	QZFL.widget.seed = {
		...
		update: function(k, opt) {
			...
			if (typeof k ==
				"undefined") n = th._update();
			else {
				k = th.prefix + k;
				if (opt && opt.useCookie) {
					n = QZFL.cookie.get(k);
					if (n) QZFL.cookie.set(k, ++n, opt.domain || th.domain, null, 3E3);
					else return 0
				} else {
					s = QZFL.shareObject.getValidSO();
					if (!s) n = th._update();
					else if (n = s.get(k)) s.set(k, ++n);
					else return 0
				}
			}
			return n
		...
		_update: function() {
			...
			QZFL.cookie.set("randomSeed", th._seed = parseInt(Math.random() * 1E6, 10), th.domain, null, 3E3);
			return th._seed
		},
		get: function(k, opt) {
			var s, n, th = QZFL.widget.seed;
			if (typeof k == "undefined") return (th._seed =
					QZFL.cookie.get("randomSeed")) ? th._seed : th.update();
			else {
				k = th.prefix + k;
				if (opt && opt.useCookie) return (n = QZFL.cookie.get(k)) ? n : (QZFL.cookie.set(k, n = 1, opt.domain || th.domain, null, 3E3), n);
				else {
					if (!(s = QZFL.shareObject.getValidSO())) return th._seed;
	...
	QZFL.widget.runBox = function(startNode, endNode, opts) {
		...
		if (!QZFL.lang.isNode(startNode) || !QZFL.lang.isNode(endNode) || !QZFL.effect) return;
		opts =
			opts || {};
	...
	(function(w) {
		...
		w.getShareObjectPrefix =
			getShareObjectPrefix
	})(window);
	if (!QZFL.pluginsDefine) QZFL.pluginsDefine = {};
	...
	(function() {
		var t = QZONE.JSONGetter,
			jsRE = /\.js|\.json$/i;
		if (t && t.pluginsPool) t.pluginsPool.srcStringHandler.push(function(ss) {
				var sw, pn;
				if (typeof ss == "string") 
					if (ss.indexOf("g_tk=") < 0) {
						pn = (sw = ss.indexOf("?") > -1) ? ss.split("?")[0] : ss;
						if (jsRE.lastIndex = 0, !jsRE.test(pn)) ss += (sw ? "&" : "?") + "g_tk=" + QZFL.pluginsDefine.getACSRFToken()
					}
				return ss
			})
	})();
	...
	QZFL.pluginsDefine.networkChectLibPath =
		"http://" + QZFL.config.resourceDomain + "/qzone/v6/troubleshooter/network_check_plugin_lib.js";
	(function() {
		var t = QZONE.FormSender;
		if (t && t.pluginsPool) t.pluginsPool.onErrorHandler.push(function(fsObj) {
	...
	(function() {
		var t = QZONE.JSONGetter;
		if (t && t.pluginsPool) t.pluginsPool.onErrorHandler.push(function(jgObj) {
	...
	(function(q) {
		...
		var commurl = "http://c.isdspeed.qq.com/code.cgi",
			...
			collector = [],
			timer, hours = (new Date).getHours(),
			rateNumG = hours > 0 && hours < 8 ? 1 : 2E3,
			rateNumP = hours > 0 && hours < 8 ? 1 : 100,
			isreportG = Math.random() * rateNumG < 1,
			isreportP = Math.random() * rateNumP < 1,
		...
		function valueStat(domain, cgi, type, code, time, rate, uin, exts) {
			...
			if (domain != "" || i > 0) q.pingSender && q.pingSender(commurl + "?" + param.join("&"), 1E3)
		}
		function _r() {
			...
			if (q[n] && q[n].pluginsPool) {
				...
				q[n].pluginsPool.onRequestComplete.push(function(th) {
					...
					if (th.msg && th.msg.indexOf("Connection") > -1) {
						collector.push([domain, url,
							2, th.statusCode || 502, +th.endTime - th.startTime, 1
						]);
						return
					}
					var d = th.resultArgs;
					if (d && (d = d[0])) if (typeof d.code == "undefined") return;
						else
					if (d.code != 0) collector.push([domain, url, 3, d.subcode || 1, +th.endTime - th.startTime, 1]);
					else {
						if (th instanceof q.JSONGetter) if (th.reportRate)(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumG]);
							else isreportG && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate ||
										rateNumG
								]);
						if (th instanceof q.FormSender) if (th.reportRate)(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumP]);
							else isreportP && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumP])
					}
				});
				q[n].pluginsPool.onRequestComplete.push(function(th) {
					if (isspeedup && th instanceof q.FormSender && (th._uri || th.uri).indexOf("taotao.qq.com") > -1) {
						var stat_url = "http://isdspeed.qq.com/cgi-bin/r.cgi?flag1=175&flag2=363&flag3=51",
							takes = th.postTime - th.startTime,
							s;
						if (ua.ie == 8) s = "2";
						else if (ua.ie == 9) s = "3";
						else if (ua.ie == 10) s = "4";
						else if (ua.chrome) s = "5";
						else if (ua.safari) s = "6";
						else if (ua.opera) s = "7";
						else if (ua.firefox) s = "8";
						stat_url += "&" + s + "=" + takes;
...
if (QZFL.userAgent.ie) eval((typeof document.documentMode == "undefined" || document.documentMode < 9 ? "var document = QZFL._doc;" : "") + "var setTimeout = QZFL._setTimeout, setInterval = QZFL._setInterval");

try {
	(function(_w) {
		_w._javascript_file_map = _w._javascript_file_map || {};
		_w._javascript_file_map['ac/qzone/qzfl/qzfl_2.1.32.js'] = true;
	})(window);
} catch (ign) {}
/*  |xGv00|e46f96ce6b59dbb13500c1806b15aa34 */