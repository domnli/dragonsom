...
(function(){
	...
	var c=window,w=document,x=w.body,u=w.documentElement,a={},d="",g=w.title,J=[],q="",b={},v=w.getElementById("bdshare_js").attributes.data,
		m=/MSIE 6.0/.test(navigator.userAgent),I=w.compatMode!="CSS1Compat",v=v.nodeValue.replace(/&amp;/g,"&").split("&")||"";
	try{
		...
		setTimeout(function(){
			...
			d=encodeURIComponent(a.href)},0)
	}
	for(var F=v.length;F--;){
		var C=v[F].split("=");
		J[C[0]]=C[1]
	}
	var z={bdPref:"bds_",bdNums:J.mini?7:15,bdMini:J.mini||false,bdType:J.type||"",bdLeft:J.pos||"right",bdTitle:"\u5206\u4eab\u5230",
			bdText:g,weiboText:"",bdMore:"\u66f4\u591a...",bdPopTitle:"\u5206\u4eab\u5230",bdImg:J.img||0,bdImgW:24,bdTop:0,bdIds:"bdshare",
			bdIdsl:J.mini?"bdshare_m":"bdshare_l",bdUid:J.uid||"",bdPic:"",searchPic:J.searchPic||0,bdComment:"",bdDesc:"",wbUid:"",tongji:"on",
			review:"on",render:true,snsKey:{},bdHost:"http://bdimg.share.baidu.com/static/",bdJump:"http://share.baidu.com/",
			bdApi:"http://api.share.baidu.com/",bdStatistics:"http://share.baidu.com/commit",bdCommit:"http://s.share.baidu.com/",
			bdMshare:"http://s.share.baidu.com/mshare",bdCout:"http://nsclick.baidu.com/v.gif?pid=307",bdWList:"([http|https]://[a-zA-Z0-9_.]+.baidu.com)",
		...
		h={ _parseJson:function(E){
						...
						for(var l=0,B=D.length;l<B;l++){
							...
							var s=N.indexOf(":");
							if(s){
								var M=N.substring(0,s).replace(/^\s*[\'\"]?|[\"\']?\s*$/g,"");
			...
			callByBrowser:function(D,B,l){
							var k=document.createElement("SCRIPT"),s=0,L=l||{},j=L.charset,K=B||function(){},E=L.timeOut||0,i;
			...
			html:function(i,e){
					...
					if(e=="link"){
						...
						return x.insertBefore(k,(e=="iframe"?x.firstChild:q.nextSibling))
					}
				},
			list:function(j){
					var k=z.bdPref,e=j.split(","),B=e[1],i="#",l=k+e[0]+" "+e[2];
					return{
						name:B,url:i,cls:l
					}
				},
			list_s:function(K){
						var E=[],l=inner="",B=z,e=B.bdList,j=B.bdNums,D=(B.bdNums<e.length)?'<li><a href="#" class="bds_more">'+B.bdMore+"</a></li>":"";
						while(j--){
							l=h.list(e[j]),E.push('<a href="'+l.url+'" class="'+l.cls+'">'+l.name+"</a>")
						}
						inner='<iframe id="bdsIfr" style="position:absolute;display:none;z-index:9999;" frameborder="0"></iframe>';
						inner+='<div id="'+B.bdIdsl+'"><div id="'+B.bdIdsl+'_c"><h6>'+B.bdTitle+"</h6>";
						inner+="<ul><li>"+E.reverse().join("</li><li>")+"</li>"+D+'</ul><p><a href="#" class="goWebsite">\u767e\u5ea6\u5206\u4eab</a></p></div></div>';
						if(K){
							var k=h.html({id:"bdshare_s",innerHTML:inner,style:"display:none"});
			...
			scroll:function(e,i){
						h.on(c,"scroll",function(s){
											var j=h.getSize(),k=z,l=k.bdTop;
											if(k.bdType=="slide"){
												if(m||I){
													h.setAttr(w.getElementById(k.bdIdsl),"display:block");
													_bdS.style.top=(j.t+(l==0?(j.h/2-(w.getElementById(k.bdIdsl).offsetHeight-11)/2):parseInt(l)))+"px"
												}else{
													if(k.bdTop!=0){
														_bdS.style.top=parseInt(k.bdTop)+"px"
													}
												}
											}
											if(i=="pop"&&(m||I)){
												e.style.top=(j.t+j.h/2-e.offsetHeight/2)+"px";
			...
			getSize:function(){
						return{
							t:(x.scrollTop||u.scrollTop),l:(x.scrollLeft||u.scrollLeft),w:(u.clientWidth||x.clientWidth),h:(c.innerHeight||u.clientHeight||x.clientHeight)
						}
					},
			getPosition:function(Q){
							var T=h.getSize(),j=Q,O={t:0,l:0},i=/gecko/.test(navigator.userAgent),N=function(s,e){O.t+=s,O.l+=e};
							if(j&&j!=x){
								if(j.getBoundingClientRect){
									var P=j.getBoundingClientRect(),R=Q.ownerDocument,D=R.body,l=R.documentElement,
										k=l.clientTop||D.clientTop||0,L=l.clientLeft||D.clientLeft||0;
									if(P.top==P.bottom){
									...
									while(j){
										...
										if(i){
											var E=parseInt(K.getPropertyValue("border-left-width"),10)||0,S=parseInt(K.getPropertyValue("border-top-width"),10)||0;
											N(S,E);
											if(j!=Q&&K.getPropertyValue("overflow")!="visible"){
												N(S,E)
											}
										}
										j=j.offsetParent
			...
			children:function(j){for(var e=[],i=j.firstChild;i;i=i.nextSibling){if(i.nodeType==1){e.push(i)}}return e},
			generateRandom:function(B,D){var e=B.length;var k="";for(var j=1;j<=D;j++){var l=Math.floor(e*Math.random());k=k+B.charAt(l)}return k},
			generateLinkid:function(){var i=(+new Date).toString(36);var e=h.generateRandom("0123456789abcdefghijklmnopqrstuvwxyz",3);return i+e},
			contains:function(i,j){if(!j||!i){return false}if(i==j){return true}var e=j.parentNode;while(e!=null&&typeof(e.tagName)!="undefind"){if(e==i){return true}e=e.parentNode}return false},
			getMousePos:function(e){if(e.pageX||e.pageY){return{x:e.pageX,y:e.pageY}}if(document.documentElement&&document.documentElement.scrollTop){return{x:e.clientX+document.documentElement.scrollLeft-document.documentElement.clientLeft,y:e.clientY+document.documentElement.scrollTop-document.documentElement.clientTop}}else{if(document.body){return{x:e.clientX+document.body.scrollLeft-document.body.clientLeft,y:e.clientY+document.body.scrollTop-document.body.clientTop}}}},
			_isCookieValidKey:function(e){return(new RegExp('^[^\\x00-\\x20\\x7f\\(\\)<>@,;:\\\\\\"\\[\\]\\?=\\{\\}\\/\\u0080-\\uffff]+\x24')).test(e)},
			getCookieRaw:function(i){if(h._isCookieValidKey(i)){var j=new RegExp("(^| )"+i+"=([^;]*)(;|\x24)"),e=j.exec(document.cookie);if(e){return e[2]||null}}return null},
			setCookieRaw:function(j,k,i){if(!h._isCookieValidKey(j)){return}i=i||{};var e=i.expires;if("number"==typeof i.expires){e=new Date();e.setTime(e.getTime()+i.expires)}document.cookie=j+"="+k+(i.path?"; path="+i.path:"")+(e?"; expires="+e.toGMTString():"")+(i.domain?"; domain="+i.domain:"")+(i.secure?"; secure":"")},
			getCookie:function(e){var i=h.getCookieRaw(e);if("string"==typeof i){i=decodeURIComponent(i);return i}return null},
			setCookie:function(i,j,e){h.setCookieRaw(i,encodeURIComponent(j),e)}
		},
		o=function(e){
				h.html({href:z.bdHost+"css/bdsstyle.css?cdnversion=20121228",rel:"stylesheet",type:"text/css"},"link");
				q=q||h.html({style:"display:none;"+(e||""),frameBorder:0},"iframe")
		},
		t=function(K,O,N,e){
				var M=/(#\d+\-[a-zA-Z\d]+\-\d+\-\d+\-[a-f\d]{32}$)/g,s=(a.search+a.hash).replace(/&/g,"#"),j=s.match(M)||"",P=new RegExp(z.bdWList,"ig"),
					E=a.protocol+"//"+a.hostname+a.pathname+a.search;
				if(P.test(a.href)&&j==""&&typeof K=="undefined"){
					return
				}
				var L=[];
				...
				if(bdShare.fn.conf.bdType){
					L.push(bdShare.fn.conf.bdType)
				}
				var D=(function(){
							for(var T=0,R=L.length,U={},S=[];T<R;T++){
								!(L[T] in U)&&S.push(L[T])&&(U[L[T]]="")
							}
							return S.join(",")
						}
						)();
				if(z.tongji=="on"||j!=""||typeof K!="undefined"){
					if(j!=""&&K!="3072"){
						j=j.toString().replace("#","");
						(new Image()).src=z.bdCommit+"back?url="+encodeURIComponent(E)+"&sign="+encodeURIComponent(j)+"&title="+encodeURIComponent(g.substr(0,300))
					}
					var k=bdShare.loadTime?(+new Date-bdShare.loadTime):"";
					if(typeof K!="undefined"){
						(new Image()).src=z.bdCout+"&type="+K+"&sign="+j+"&uid="+z.bdUid+"&desturl="+encodeURIComponent(w.referrer)+"&linkid="+f+"&button="+D+"&loadTime="+k
					}else{
						var Q=[0,0,0,0];
						for(var l=0,B=L.length;l<B;l++){
							Q[{tools:0,button:1,slide:2}[L[l]]]=1
						}
						bdShare._LogPool=bdShare._LogPool||[];
						bdShare._LogPool.push({key:"cite",api:{uid:z.bdUid||"",linkid:f,type:parseInt(Q.reverse().join(""),2),mini:z.bdMini?1:0,loadTime:k}})
					}
				}
			},
		G={
			create:function(){var E=z,L=E.bdList,i=L.length,k=[],j="",M=h.getSize(),e=w.getElementById(E.bdIds+"_pop"),D=((m||I)?"absolute":"fixed"),K=((m||I)?M.t:0);if(!e){while(i--){j=h.list(L[i]),k.push('<a href="'+j.url+'" class="'+j.cls+'">'+j.name+"</a>")}var B=h.html({id:E.bdIds+"_pop",style:"position:"+D,innerHTML:"<div><h5></h5><ul><li>"+k.reverse().join("</li><li>")+'</li></ul><p><a href="#" class="goWebsite">\u767e\u5ea6\u5206\u4eab</a></p></div>'});A.bind(B,"mouseover","a");A.bind(B,"click","b",function(){B.style.display="none";q.style.display="none"});h.on(B,"mouseover",function(R){var R=R||window.event;var Q=R.target||R.srcElement;var P=h.getPosition(Q);var N=h.getMousePos(R);var s=N.x-P.l;var S=N.y-P.t;Q.buttontype=2;Q.x=s;Q.y=S;var O=R.relatedTarget||R.fromElement;if(h.contains(B,O)){return}B.time=+new Date})}var l=B||e;l.getElementsByTagName("h5")[0].innerHTML=(b.title||E.bdPopTitle)+"<b>&nbsp;</b>";h.setAttr(l,"display:block;");h.setAttr(l,"left:"+(M.w/2-l.offsetWidth/2)+"px;top:"+(K+M.h/2-l.offsetHeight/2)+"px");h.setAttr(q,"position:"+D+";display:block;z-index:10000;width:"+l.offsetWidth+"px;height:"+l.offsetHeight+"px;left:"+l.style.left+";top:"+l.style.top);h.scroll(l,"pop");h.on(c,"resize",function(O){var s=h.getSize(),N="left:"+(s.w/2-l.offsetWidth/2)+"px;top:"+(K+s.h/2-l.offsetHeight/2)+"px";h.setAttr(l,N);h.setAttr(q,N)});if(E.bdType!="slide"){w.getElementById(z.bdIdsl).style.display="none";w.getElementById("bdsIfr").style.display="none"}window.bdShareActivity&&bdShareActivity.check()}
		},
		A={
			bind:function(i,e,l,k){var j=this;i.addEventListener?i.addEventListener(e,function(s){return j._action(s,s.target,l,i,k)},false):i.attachEvent("on"+e,function(s){s=c.event;return j._action(s,s.srcElement,l,i,k)})},
			_action:function(B,i,D,s,l){if(i.nodeName==D.toUpperCase()){if(D=="a"||D=="span"){var E=i.className,j=E=="goWebsite"?E:E.split(" ")[0].split("_")[1],k=this;if(j){(j=="more"&&D=="span")?k._tools():i.onclick=function(K,e){return function(P){var P=P||window.event;var O=P.target||P.srcElement;var N=h.getPosition(O);var M=h.getMousePos(P);var L=M.x-N.l;var Q=M.y-N.t;if(K=="more"){k._tools()}else{if(K=="copy"){k._copy(k)}else{if(K=="print"){k._print()}else{k._jump(K,e,i,L,Q)}}}return false}}(j,s)}return false}if(typeof l==="function"){l()}}if(typeof B.preventDefault==="function"){B.preventDefault();B.stopPropagation()}else{B.returnValue=false;B.cancelBubble=true}},
			_tools:function(){t(3072);G.create()},
			_copy:function(i){_this=i;var e=new _this._browsers();if(e.name=="IE"){window.clipboardData.setData("text",document.title+" "+a.href);alert("\u6807\u9898\u548c\u94fe\u63a5\u590d\u5236\u6210\u529f\uff0c\u60a8\u53ef\u4ee5\u63a8\u8350\u7ed9QQ/MSN\u4e0a\u7684\u597d\u53cb\u4e86\uff01")}else{window.prompt("\u60a8\u4f7f\u7528\u7684\u662f\u975eIE\u6838\u5fc3\u6d4f\u89c8\u5668\uff0c\u8bf7\u6309\u4e0b Ctrl+C \u590d\u5236\u4ee3\u7801\u5230\u526a\u8d34\u677f",document.title+" "+a.href)}},
			_mail:function(e){},
			_print:function(){var e=document.getElementById("bdshare_pop");e&&(e.style.display="none");window.print();e&&(e.style.display="block");return},
			_jump:function(am,al,e,V,U){
						var ak=z,T=ak.bdPic,B=ak.bdText,R=ak.weiboText,ac=ak.wbUid,D=d,ab=ak.bdDesc,Q=ak.bdComment,O=ak.bdMiniWindow||0;
						b=(al.getAttribute("data")&&al.getAttribute("data")!="")?h._parseJson(al.getAttribute("data")):b;
						if(b!=""){
							T=encodeURIComponent(b.pic||"")||T;
							D=encodeURIComponent(b.url||"")||D;
							Q=encodeURIComponent(b.comment||"")||Q;
							ab=encodeURIComponent(b.desc||"")||ab;
							ac=b.wbuid||ac;
							if(am=="tsina"){
								if(b.text&&b.weibotext){
									B=encodeURIComponent((b.text+b.weibotext).substr(0,300))
								}else{
									if(b.weibotext){
										B=encodeURIComponent((B+b.weibotext).substr(0,300))
									}else{
										if(b.text){
											B=encodeURIComponent((b.text+R).substr(0,300))
										}else{
											B=encodeURIComponent((B+R).substr(0,300))
										}
									}
								}
							}else{
								B=encodeURIComponent((b.text||"").substr(0,300))||encodeURIComponent(B.substr(0,300))
							}
						}else{
							if(am=="tsina"){
								B=encodeURIComponent((B+R).substr(0,300))
						...
						var ae=r.toString(32)+p.toString(32)+n.toString(32);
						var ao=e.buttontype;
						var k=Math.floor(bdShare.XY.loadedX);
						var j=Math.floor(bdShare.XY.loadedY);
						var N=ao>0?1:0;
						var Y=Math.floor(e.x);
						var W=Math.floor(e.y);
						var K=Math.floor(V);
						var E=Math.floor(U);
						var X=Math.floor(e.offsetWidth);
						var ah=Math.floor(e.offsetHeight);
						var M=ao;
						var Z=+new Date;
						Z=Z-al.time;
						var L=[k,j,N,Y,W,K,E,X,ah,M,Z].join(".");
						var i;
						switch(z.bdType){
							case"button":i=1;
								break;
						...
						var an=am=="mail"?"_mail":am;
						var P="?click=1&url="+D+"&uid="+ak.bdUid+"&to="+an+"&type=text&relateUid="+ac+"&pic="+T+"&title="+B+"&key="+(ak.snsKey[am]||"")+"&sign="+ak.review+"&desc="+ab+"&comment="+Q+"&searchPic="+ak.searchPic+"&l="+ae+"&linkid="+f+"&sloc="+L+"&apiType=0&buttonType="+i+"&firstime="+h.getCookie("bdshare_firstime");
						if(am=="mshare"){
							var aj=ak.bdMshare+P
						}else{
							var aj=ak.bdCommit+P
						}
						var S=ak.bdCommit+"commit"+P+"&t="+Math.random();
						if(am!="goWebsite"){
							...
							setTimeout(function(){ag(S)},1500);
							if(am=="bdxc"){
								var ai=window,aj=document,ad="_bdXC",aa;
								if(ai[ad]){
									if(window._bdXC_loaded){
										ai[ad].reInit()
									}
								}else{
									aa=aj.createElement("script");
									aa.setAttribute("charset","utf-8");
									aa.src="http://xiangce.baidu.com/zt/collect/mark.js?"+(new Date()).getTime();
									aj.getElementsByTagName("head")[0].appendChild(aa)
								}
							}else{
								if(am=="mail"){
									var af=z.bdCommit+"sendmail";
									window.open(af+P,"_blank")
								}else{
									if(1==O){
										c.open(aj,"bdShare","height=400, width=600")
									}else{
										c.open(aj)
									}
								}
							...
							init:function(){
								...
								if(typeof bds_config!="undefined"){
									for(var K in bds_config){
										if(K=="bdPopTitle"||K=="bdText"||K=="weiboText"||K=="bdPic"||K=="wbUid"||K=="tongji"||K=="render"||K=="snsKey"||K=="review"||K=="bdComment"||K=="bdDesc"||K=="bdTop"||K=="searchPic"||K=="bdUrl"||K=="bdMiniWindow"){
											z[K]=(K=="render"||K=="snsKey"||K=="bdPopTitle"||K=="bdText"||K=="weiboText")?bds_config[K]:encodeURIComponent(bds_config[K])
										}
									}
								}
								this.conf=z;
								...
								for(var B=0;B<D.length;B++){
									...
									for(var s=0;s<E.length;s++){
										!(E[s].getAttribute("href"))&&E[s].setAttribute("href","#")
									}
								}
								...
								if(h.getCookie("bdshare_firstime")==null){
									h.setCookie("bdshare_firstime",new Date()*1,{path:"/",expires:new Date().setFullYear(2022)-new Date()})
								}
							},
							_getSc:function(i){
										this.type=i;
										var j=w.querySelectorAll?w.querySelectorAll(".shareCount"):w.getElementsByTagName("*"),e=j.length;
										while(e--){
											if(/\sshareCount|shareCount\s|shareCount$/.test(j[e].className)){
												this.s.push(j[e])
											}
										}
										if(this.s.length>0){
											h.callByBrowser(this.conf.bdApi+"getnum?url="+d+"&callback=bdShare.fn._getShare&type="+i+"&t="+new Date().getTime())
										}
									},
							_getShare:function(B){
											...
											if(w.querySelectorAll){
												...
												while(j--){
													...
													if(/\sbdshare_b|bdshare_b\s|bdshare_b$/.test(D.className)){
														this.b.push(D)
													}
												}
											}
											if(k){
												...
												while(i--){
													if(/\sshareCount|shareCount\s|shareCount$/.test(L[i].className)){
														E=this.type=="share"?(B.num[0]<10000?parseInt(L[i].innerHTML)+1:B.num[1]):(B.errno*1==0?B.num[1]:0);
														L[i].innerHTML=decodeURI(E);
														L[i].setAttribute("title","\u7d2f\u8ba1\u5206\u4eab"+B.num[0]+"\u6b21");
